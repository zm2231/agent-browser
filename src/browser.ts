...
export class BrowserManager {
  ...
  async launch(options: LaunchCommand): Promise<void> {
    ...
    const useStealth =
      process.env.AGENT_BROWSER_STEALTH === '1' || process.env.AGENT_BROWSER_STEALTH === 'true';
    ...
    const launcher =
      useStealth && browserType === 'chromium'
        ? stealthChromium
        : browserType === 'firefox'
          ? firefox
          : browserType === 'webkit'
    ...
    if (hasExtensions) {
      ...
      context = await launcher.launchPersistentContext(
        path.join(os.tmpdir(), `z-agent-browser-ext-${session}`),
        {
          headless: false,
   ...
   * @param url - Optional URL to navigate to after creating the tab
   */
  async newTab(
    url?: string
  ): Promise<{ index: number; total: number; url?: string; title?: string }> {
    ...
    const result: { index: number; total: number; url?: string; title?: string } = {
  ...
  async startRecording(outputPath: string, url?: string): Promise<void> {
    ...
    this.recordingTempDir = path.join(
      os.tmpdir(),
      `z-agent-browser-recording-${session}-${Date.now()}`
    );
